...
var slowestExecution = 0;
var slowestSampleAt = null;
var slowestSampleRate = 1000 * 60;
var slowestTrace = null;
...
Tracer.prototype.event = function(traceInfo, type, data) {
  ...
  traceInfo.at = traceInfo.at || Ntp._now();
...
Tracer.prototype.eventEnd = function(traceInfo, eventId, data) {
  if(traceInfo._lastEventId && traceInfo._lastEventId == eventId) {
    ...
    if (
      !traceInfo.name.includes("generateFixtures") &&
      traceInfo.events.filter(({ type }) => type === "async").length >= 1
    ) {
      var executionTimeMs = event.at - traceInfo.at;

      if (executionTimeMs > slowestExecution) {
        slowestExecution = executionTimeMs;
        slowestSampleAt = slowestSampleAt || new Date();
        slowestTrace = traceInfo;
      }
    }

    if (slowestSampleAt && new Date(slowestSampleAt.getTime() + slowestSampleRate) < new Date()) {
      if (Meteor.settings.adiraskadira && Meteor.settings.adiraskadira.logSlowTraces) {
        console.info(`${slowestExecution}ms`, slowestTrace)
      }

      slowestExecution = 0;
      slowestSampleAt = null;
      slowestTrace = null;
    }
